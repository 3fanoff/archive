...
(function($){
	$.fn.carouselme = function(options){
		var defs = {
			visible: 4,
			start: 1,
			stretch: false, //stretching elements when number of elements less visibles
			speed: 500,
			type: 'simple',
			circle: false,
			autoplay: false,
		...
		return this.each(function(i){
			var opts = $.extend({}, defs, options),
				...
				el_p = el.outerWidth(true) - el.width(),
				el_num = el.size(),
				el_pos[i] = 0,
				el_vis = opts.visible,
				active = true;
			if (opts.stretch && el_vis > el_num){
				el_vis = el_num;
			}
			...
			el_w = Math.round(elbox_w / el_vis) - el_p;
			curr[i] = -((0 + start) * (el_w + el_p));
				
			elbox.wrapAll('<div class="' + opts.elbox_cls + '"/>');
			$this.children('.' + opts.elbox_cls).css({'width': elbox_w, 'overflow-x': 'hidden', 'position': 'relative'});
			el.css({'width': el_w});
			elbox.css({'width': el_num * (el_w + el_p), 'position': 'relative', 'left': curr[i]});
			if (opts.generatenav && opts.visible < el_num){
			...
			if (opts.type == 'simple' || typeof opts.type == 'undefined' || opts.type == ''){
				max_pos[i] = el_num * (el_w + el_p) - el_vis * (el_w + el_p);
				function act(dir){
					if (opts.circle){
						...
						if (dir == 'p' && el_pos[i] == curr[i]){
							last.after(first);
							elbox.css({left: el_pos[i] + (el_w + el_p)})
				...
				function prev(){
					active = false;
					if (el_pos[i] <= -max_pos[i]){
						gts == 1 ? el_pos[i] = 0 : el_pos[i] = curr[i];
					} else {
						el_pos[i] = curr[i] - (el_w + el_p);
					}
				...
				function next(){
					active = false;
					if (curr[i] == 0){
						gts == 1 ? el_pos[i] = -max_pos[i] : el_pos[i] = 0;
					} else {
						el_pos[i] = curr[i] + (el_w + el_p);
					}
			...
			function hold(holder){
				...
				if (holder == 'p'){
					el_pos[i] <= -max_pos[i] ? $('.' + opts.prev_cls, $this).addClass(opts.dis_cls) : $('.' + opts.nav_cls, $this).removeClass(opts.dis_cls);
				}
			}
			function pause(){
